...
public class MessageHandler {
	private enum Command {
		INVALID_PROTOCOL, CONNECTION_REFUSED, HANDSHAKE_REQUEST, HANDSHAKE_RESPONSE, FILE_CREATE_REQUEST, FILE_CREATE_RESPONSE, FILE_DELETE_REQUEST, FILE_DELETE_RESPONSE, FILE_MODIFY_REQUEST, FILE_MODIFY_RESPONSE, DIRECTORY_CREATE_REQUEST, DIRECTORY_CREATE_RESPONSE, DIRECTORY_DELETE_REQUEST, DIRECTORY_DELETE_RESPONSE, FILE_BYTES_REQUEST, FILE_BYTES_RESPONSE
	}
	...
	/**
	 * handle incoming messages, unmarshall json string and call different
	 * methods according to command
	 * 
	 * @param msg
	...
	public List<Document> handleMsg(String msg) {
		log.info("Reveived: " + msg); // print received message to Log
	 ...
	 * pathName is safe. It will use a local copy if it's local directory
	 * possess a file that has the same md5 value. A FILE_CREATE_RESPONSE json
	 * is returned as well as a FILE_BYTES_REQUEST if necessary.
	 * 
	 * @param json
	 *            The incoming protocol message
	...
	/**
	 * Handle incoming 'FILR_BITYS_REQUEST'. Unmarshall needed fields and encode
	 * the content based on the Base64. Return a 'FILE_BYTES_RESPONSE' after
	 ...
	 * create a Base 64 decoder to decode the 'content' field. Write content
	 * into file and call checkWriteComplete() to check whether the file has
	 * been successfully write or not. If not finished, a new
	 * 'FILR_BITYS_REQUEST' will be returned to request further content.
	 * 
	 * @param json
	 *            The incoming protocol message
	...
	/**
	 * append general info when creating response protocol, generally it uses
	 * the json that has been received before and replace the 'commond' field,
	 * append 'message' and 'status' field
	 * 
	 * @param json
	 *            The Document object that need to modify
	 * @param cmd
	 *            The command that want to replace with
	 * @param message
	 *            A String contains returning information
	 * @param status
