...
public class FileSystemManager extends Thread {
    ...
    public boolean isSafePathName(String pathName) {
        ...
        File file = new File(
                root + FileSystems.getDefault().getSeparator() + pathName);
        ...
        return cannonicalName.startsWith(
                cannonicalRoot + FileSystems.getDefault().getSeparator())
    ...
    public boolean dirNameExists(String pathName) {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            return watchedDirectories.contains(
                    root + FileSystems.getDefault().getSeparator() + pathName);
        }
    }
    ...
    public boolean fileNameExists(String pathName) {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            return watchedFiles.containsKey(
                    root + FileSystems.getDefault().getSeparator() + pathName);
        }
    }
    ...
    public boolean fileNameExists(String pathName, String md5) {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            String fullPathName = root
                    + FileSystems.getDefault().getSeparator() + pathName;
    ...
    public boolean deleteFile(String pathName, long lastModified, String md5) {
        ...
        synchronized (this) {
            String fullPathName = root
                    + FileSystems.getDefault().getSeparator() + pathName;
            if (watchedFiles.containsKey(fullPathName) && (watchedFiles
                    .get(fullPathName).lastModified <= lastModified
                    || watchedFiles.get(fullPathName).md5.equals(md5))) {
     ...
     */
    public boolean createFileLoader(String pathName, String md5, long length,
            long lastModified) throws NoSuchAlgorithmException, IOException {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            String fullPathName = root
                    + FileSystems.getDefault().getSeparator() + pathName;
            ...
            loadingFiles.put(fullPathName,
                    new FileLoader(fullPathName, md5, length, lastModified));
        }
        return true;
    }
     ...
     */
    public boolean writeFile(String pathName, ByteBuffer src, long position)
            throws IOException {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            String fullPathName = root
            ...
            if (hashMap.containsKey(md5)) {
                for (String attempt : hashMap.get(md5)) {
                    try {
                        ...
                        String currentMd5 = hashFile(file, attempt,
                                watchedFiles.get(attempt).lastModified);
                        if (currentMd5.equals(md5)) {
                            ...
                            if (read < length)
                                throw new IOException(
                                        "did not read everything expected");
                            return bb;
                        }
                        lock.release();
                        channel.close();
     ...
     *                                  this case.
     */
    public boolean checkWriteComplete(String pathName)
     ...
     *                                  this case.
     */
    public boolean checkShortcut(String pathName)
            throws NoSuchAlgorithmException, IOException {
        pathName = separatorsToSystem(pathName);
        synchronized (this) {
            String fullPathName = root
     ...
     */
    public boolean modifyFileLoader(String pathName, String md5,
        ...
        synchronized (this) {
            ...
            if (watchedFiles.containsKey(fullPathName) && watchedFiles
                    .get(fullPathName).lastModified <= lastModified) {
                loadingFiles.put(fullPathName,
                        new FileLoader(fullPathName, md5,
    ...
    public ArrayList<FileSystemEvent> generateSyncEvents() {
        synchronized (this) {
            ...
            for (String pathname : keys) {
                ...
                pathevents.add(new FileSystemEvent(file.getParent(),
                        file.getName(), EVENT.DIRECTORY_CREATE));
            }
            ...
            for (String pathname : keys) {
                File file = new File(pathname);
                pathevents.add(new FileSystemEvent(file.getParent(),
                        file.getName(), EVENT.FILE_CREATE,
    ...
    private class FileLoader {
        ...
        public FileLoader(String pathName, String md5, long length,
                long lastModified) throws IOException {
        ...
        public void writeFile(ByteBuffer src, long position)
                throws IOException {
            if (position > length)
                throw new IOException(
                        "trying to write bytes beyond what is expected");
            ...
            synchronized (this) {
                ...
                for (String pathname : keys) {
                    ...
                    if (!file.exists()) {
                        ...
                        dropFile(pathname);
                        pathevents.add(new FileSystemEvent(file.getParent(),
                                file.getName(), EVENT.FILE_DELETE, fdes));
                    }
                }
                ...
                for (String pathname : keys) {
                    ...
                    if (!file.exists()) {
                        dropDir(pathname);
                        pathevents.add(new FileSystemEvent(file.getParent(),
                                file.getName(), EVENT.DIRECTORY_DELETE));
                    }
                }
            }
    ...
    private String hashFile(File file, String name, long lastModified)
            throws NoSuchAlgorithmException, IOException {
        ...
        if (file.isFile()) {
            ...
            if (watchedFiles.containsKey(name)) {
                if (lastModified != watchedFiles.get(name).lastModified) {
                    ...
                    FileSystemEvent pe = new FileSystemEvent(file.getParent(),
                            file.getName(), EVENT.FILE_MODIFY,
                ...
                String newHash = hashFile(file, name, 0);
                addFile(name,
                        new FileDescriptor(lastModified, newHash, fileSize));
                FileSystemEvent pe = new FileSystemEvent(file.getParent(),
                        file.getName(), EVENT.FILE_CREATE,
                        watchedFiles.get(name));
                pathEvents.add(pe);
    ...
    private void modifyFile(String name, String md5, long lastModified,
            long fileSize) {
    ...
    private static String getFileChecksum(MessageDigest digest,
            RandomAccessFile fis) throws IOException {
