...
/**
 * Unlike the TCP mode, UDP mode unifies both server and client and forms an agent handling receiving/sending messages,
 * which is more like a peer. However, only those remembered peers(which successfully passed through the handshake process)
 * are able to communicate with the local peer.
 * @author Kiwilyc
 *
 */
